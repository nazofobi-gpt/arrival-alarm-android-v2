...
class TripInferenceUiTest {
    ...
    @Test fun inferenceIsOptInVisibleAndConfirmationDoesNotArmAlarm() {
        ...
        rule.onNodeWithTag("trip-inference-run").performScrollTo().performClick()
        rule.waitForIdle()
        rule.onNodeWithTag("trip-inference-result").performScrollTo()
            .assertTextContains("Flughafen", substring = true).assertIsDisplayed()
        rule.onNodeWithTag("trip-inference-confirm").performScrollTo().performClick()
        rule.onNodeWithTag("trip-inference-confirmed").performScrollTo()
            .assertTextContains("hedef/alarm ayrıca", substring = true).assertIsDisplayed()
